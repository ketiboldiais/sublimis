...
_Step 1: Turn the message into some prime number._ For example, consider
the string `"victory"`. We can represent each letter in this string with an
integer corresponding to its index in the Latin alphabet sequence. For
example, the letter `v` is the twenty-second letter of the alphabet, `i` is
the ninth, `c` is the third, and so on. Mapping all the letters to their
indices:
...
$$
	22090320151825\textcolor{red}{13}
$$
